...
function custom_rewrite_rules() {
	...
	add_rewrite_rule(
		'^usthb/([^/]+)/([^/]+)?$',
		'index.php?post_type=speciality&name=$matches[1]&semester_var=$matches[2]',
		'top'
	);
...
function custom_permalinks( $permalink, $post ) {
	if ( $post->post_type == 'course' ) {
		...
		$module          = new Module( $post->ID );
		$module_slug     = $module->get_slug();
		$speciality_slug = $module->get_speciality()->get_slug();
		$semester        = $module->get_semester();
		$permalink       = trailingslashit( home_url( "usthb/$speciality_slug/$semester/$module_slug" ) );
	} elseif ( $post->post_type == 'speciality' ) {
        $speciality = new Speciality($post->ID);
        $speciality_slug = $speciality->get_slug();
		$permalink       = trailingslashit( home_url( "usthb/$speciality_slug" ) );
